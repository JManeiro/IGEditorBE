...
local function SetForcedPlots(city, forcedPlots)
	for _, v in ipairs(forcedPlots) do
		local index = city:GetCityIndexPlot(v);
		if index > 0 then
...
local function UpdateOwnershipForCityPlot(player, city, plot)
	...
	if city:IsWorkingPlot(plot) then
		city:AlterWorkingPlot(city:GetCityIndexPlot(plot));
	end
...
local function UpdateOwnership(plot)
	...
	if owner ~= nil then
		...
		if city and city:Plot() ~= plot then
			...
			print("UpdateOwnerShipcityplot: ",owner," ",city," ",plot);
		else
			plot:SetOwner(-1);
...
-- Temporary deploy worker unit to update improvement plot
function RefreshRevealPlot(plot)
	local pPlayer = Players[IGE.currentPlayerID];
	local pUnit;
	pUnit = pPlayer:InitUnit(3, plot:GetX(), plot:GetY());
	pUnit:Kill();
end
...
function CanHaveImprovement(plot, improvement)
	...
	if improvement.hillsMakesValid then
		if plot:GetPlotType() == PlotTypes.PLOT_HILLS then return true end
	end

	if improvement.freshWaterMakesValid then
		if plot:IsFreshWater() then return true end
	end
...
function SetFeature(feature, plot)
	if plot and plot:GetFeatureType() ~= feature.ID then
		...
		--- Add miasma
		if not plot:HasMiasma() and feature.type == "FEATURE_MIASMA" then
			if plot:CanHaveMiasma() then
				plot:SetMiasma(true);
				plot:SetArea(-1)
				return true, true;
			else
				print("This plot cannot have miasma");
				return true, true;
			end
		end
		--- Remove miasma + other features
		if plot:HasMiasma() and feature.ID == -1 then
	...
	-- remove miasma on miasma only plots
	if plot:HasMiasma() and feature.ID == -1 then
				plot:SetMiasma(false);
				plot:SetFeatureType(feature.ID);
				plot:SetArea(-1)
